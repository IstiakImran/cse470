...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    // Get the current user for notification message
    const currentUser = await User.findById(userId);

    // Create notification for the ride owner
    await Notification.create({
      userId: updatedRide.user,
      senderId: userId,
      type: "ride_join",
      message: `${currentUser.firstName} ${currentUser.lastName} has joined your ride to ${updatedRide.destination}`,
      isRead: false,
    });
